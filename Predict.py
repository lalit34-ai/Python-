...
def get_medical_intent(transcription):
    # Split the transcription into words
    words = transcription.split()

  # Find the first word that is a medical term
    medical_term = next((word for word in words if word in MEDICAL_TERMS), None)

  # If no medical term is found, return None
    if medical_term is None:
        return None

  # Return the medical intent
    return MEDICAL_INTENTS[medical_term]


# In[3]:


MEDICAL_TERMS = ["cancer", "heart disease", "stroke", "diabetes", "alzheimer's disease"]
...
# Extract the medical intent from the transcription
medical_intent = get_medical_intent(transcription)

# Predict the disease based on the medical intent
disease = model.predict_classes([medical_intent])[0]
...
tr=pd.read_csv("C:\\Users\\lalit\\Downloads\\Testing.csv")
...
df=pd.read_csv("C:\\Users\\lalit\\Downloads\\Training.csv")
...
root.mainloop()   #https://github.com/jini-the-coder/Diseaseprediction/blob/master/disease_prediction.py
